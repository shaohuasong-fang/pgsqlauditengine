...
ALTER STATISTICS se_x_stats SET STATISTICS 100;  -- [PG13+]
...
-- [CUT PG14] MERGE INTO se_t_dml AS t USING (SELECT 3 AS id) AS s ON (t.id = s.id)  -- [PG15+]
-- [CUT PG14] WHEN NOT MATCHED THEN INSERT (id) VALUES (s.id);
-- [CUT PG14] MERGE INTO se_t_dml AS t USING (SELECT 5 AS id) AS s ON (t.id = s.id)  -- [PG15+]
-- [CUT PG14] WHEN NOT MATCHED AND s.id > 0 THEN INSERT (id, name, amount, status) VALUES (s.id, 'merge-ok', 0.00, 'ok');
