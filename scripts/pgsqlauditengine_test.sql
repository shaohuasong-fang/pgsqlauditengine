...
MERGE INTO se_t_dml AS t USING (SELECT 3 AS id) AS s ON (t.id = s.id)  -- [PG15+]
WHEN NOT MATCHED THEN INSERT (id) VALUES (s.id);
MERGE INTO se_t_dml AS t USING (SELECT 5 AS id) AS s ON (t.id = s.id)  -- [PG15+]
WHEN NOT MATCHED AND s.id > 0 THEN INSERT (id, name, amount, status) VALUES (s.id, 'merge-ok', 0.00, 'ok');
