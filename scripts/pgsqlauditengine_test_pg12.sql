...
REINDEX INDEX CONCURRENTLY se_t_base_pkey;  -- [PG12+]
...
-- [CUT PG12] ALTER STATISTICS se_x_stats SET STATISTICS 100;  -- [PG13+]
...
-- [CUT PG12] MERGE INTO se_t_dml AS t USING (SELECT 3 AS id) AS s ON (t.id = s.id)  -- [PG15+]
-- [CUT PG12] WHEN NOT MATCHED THEN INSERT (id) VALUES (s.id);
-- [CUT PG12] MERGE INTO se_t_dml AS t USING (SELECT 5 AS id) AS s ON (t.id = s.id)  -- [PG15+]
-- [CUT PG12] WHEN NOT MATCHED AND s.id > 0 THEN INSERT (id, name, amount, status) VALUES (s.id, 'merge-ok', 0.00, 'ok');
